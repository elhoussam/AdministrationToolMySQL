...
public class arbre extends JPanel {
	...
	public arbre(){
		...
		ref.setIcon( new ImageIcon( getClass().getClassLoader().getResource("icons/tree_refresh.png") )); 
		...
		CellRender.setClosedIcon( new ImageIcon( getClass().getClassLoader().getResource("icons/tree_database.png") ) );
		CellRender.setOpenIcon( new ImageIcon( getClass().getClassLoader().getResource("icons/tree_database.png") ) );
		CellRender.setLeafIcon( new ImageIcon( getClass().getClassLoader().getResource("icons/tree_table.png") ) );
		...
		rename.setIcon( new ImageIcon( getClass().getClassLoader().getResource("icons/rename.png") ) );
		rename.addActionListener( new ActionListener(){
			...
			public void actionPerformed(ActionEvent e) {
				...
				if( nv_nom != null ){
					...
					if( database ) obj = new DataBase("");
					else obj = new DataBase( SelectParent );

					// rename the object db , table
					if( database )  	query = "ALTER DATABASE "+SelectNode+" Modify Name="+nv_nom  ;
					else   query = "exec sp_rename "+ SelectNode +" , "+ nv_nom;
					if(obj.Update( query )  )
		...
		delete.setIcon( new ImageIcon( getClass().getClassLoader().getResource("icons/delete.png") ) );	
	...
	public static void showDatabase(){
		...
		try{ 
			conn.Execute("select name from sys.databases",true); 
			while( conn.getResult().next() ){ 
				String nameDB = conn.getResult().getString("name") ; 
	...
	private  void  showTable( String DB , DefaultMutableTreeNode root ) {
		...
		try{  // Get connection  
			conn.tableOf(DB);
			conn.Execute("select name from sys.tables ",true); 
			short count = 0 ;
			// get the result one by one 
			while ( conn.getResult().next() ){
				String nameof = conn.getResult().getString("name");
